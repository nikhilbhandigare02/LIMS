...
class MisDetailsScreen extends StatefulWidget {
  const MisDetailsScreen({super.key});

  @override
  State<MisDetailsScreen> createState() => _MisDetailsScreenState();
}

class _MisDetailsScreenState extends State<MisDetailsScreen>
  ...
  Future<void> exportSampleListToCsv(List<SampleList> data) async {
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      rows.add([
        ...
        item.serialNo ?? '',
        item.labLocation ?? '',
        _formatDate(item.sampleSentDate),
        _formatDate(item.sampleResentDate),
        _formatDate(item.sampleReRequestedDate),
        item.statusName ?? '',
      ]);
    }
    ...
    final filePath =
        '${directory.path}/sample_list_${DateTime.now().millisecondsSinceEpoch}.csv';
  ...
  Future<void> _fetchStatusOptions() async {
    ...
    try {
      ...
      try {
        print(
          'Status fetch - request payload (encrypted): ' +
      ...
      try {
        ...
        final String serverIvBase64 =
            (response['iv'] ?? response['IV']) as String;
        ...
        children: [
                          ...
                          SizedBox(
                            ...
                            child: Builder(
                              builder: (context) {
                                ...
                                if (_isStatusLoading) {
                                  final loading = AppLocalizations.of(
                                    context,
                                  )!.loadingStatus;
                                  return Opacity(
                                    opacity: 0.7,
                                ...
                                if (_statusOptions.isEmpty) {
                                  final none = AppLocalizations.of(
                                    context,
                                  )!.noStatusFound;
                                  return Opacity(
                                    opacity: 0.7,
                          ...
                          SizedBox(
                            ...
                            child: BlocBuilder<SampleFormBloc, SampleFormState>(
                              ...
                              builder: (context, s) {
                                ...
                                final items = [
                                  AppLocalizations.of(context)!.selectLab,
                                  ...s.labOptions,
                                ];
                                final String currentLab = selected.isEmpty
                                    ? AppLocalizations.of(context)!.selectLab
                                    : selected;
                                return BlocSearchableDropdown(
          ...
          Container(
            ...
            child: IconButton(
              ...
              tooltip: showFilters
                  ? AppLocalizations.of(context)!.filterTooltipClearHide
                  : AppLocalizations.of(context)!.filterTooltipShow,
            ),
          ),
        ],
      ),
  ...
  Color getStatusColor(String? status) {
    ...
    if (s.contains('decoded') && s.contains('report')) return Colors.indigo;
    if (s.contains('decoded')) return Colors.deepPurple;
    if (s.contains('dispatched') || s.contains('dispatch')) return Colors.teal;
    if (s.contains('generated') && s.contains('report')) return Colors.blueGrey;

    if (s.contains('verification')) return Colors.blue;
    ...
    if (s.contains('analysis')) return Colors.green;
    if (s.contains('Resubmitted')) return Colors.green;
    if (s.contains('received')) return Colors.purple;
    ...
    if (s == 'tempered' || s.contains('re-request')) return Colors.redAccent;
    ...
    return WillPopScope(
      ...
      child: BlocProvider.value(
        ...
        child: Scaffold(
          ...
          appBar: AppHeader(screenTitle: "MIS Details", showBack: false),
          drawer: CustomDrawer(),
          body: SafeArea(
            top: false,
            left: false,
              ...
              builder: (context, state) {
                switch (state.fetchSampleList.status) {
                  ...
                  case Status.complete:
                    ...
                    final String? labFilter =
                        (selectedLab == null || selectedLab!.trim().isEmpty)
                        ? null
                        : selectedLab!.trim();
                    // 1) Apply Lab filter
                            ...
                            Padding(
                              ...
                              child: Row(
                                ...
                                children: [
                                  InkWell(
                                    onTap: () {
                                      exportSampleListToCsv(filteredSampleDataList.cast<SampleList>());
                                    },
                      ...
                      : SingleChildScrollView(
                          ...
                          child: DataTable(
                            ...
                            columns: [
                              ...
                              DataColumn(
                                label: Text("Sample Name / Description"),
                              ),
                              DataColumn(label: Text("Send Date")),
                              DataColumn(label: Text("Received Date")),
                              DataColumn(label: Text("Test(s) Requested")),
                            ...
                            rows: paginatedData.map((data) {
                              return DataRow(
                                cells: [
                                  ...
                                  DataCell(
                                    Text(_formatDate(data.sampleSentDate)),
                                  ),
                                  DataCell(
                                    Container(
                                      width: 120,
            ...
            children: [
              Flexible(
                child: Text(
                  AppLocalizations.of(
                    context,
                  )!.showingRange(start, end, totalItems),
                  style: TextStyle(
                    fontSize: 12,
